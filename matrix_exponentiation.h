...
Matrix<T> matrix_powmod(Matrix<T>& m1, U exponent, U modulus){
 ...
 //Restrição a expoentes inteiros
 static_assert(std::is_integral<T>::value, "Elementos da matriz congruentes devem ser inteiros");
 static_assert(std::is_integral<U>::value, "Inteiros congruentes devem ser inteiros");
 assert(m1.rows==m1.columns);
