...
/*
USAREMOS EXPRESSÕES MATEMÁTICAS PARA OS CASOS DE GRAU 1 E PARA GRAUS MAIORES OU
IGUAIS A 2 USAREMOS O MÉTODO DE NEWTON-RAPHSON.
...
polynomial<T> polynomial_derivative(polynomial<T>& p2) {
  ...
  for (j = 0; j < p2.polynomial_coefficients.size(); ++j)
    result.polynomial_coefficients.push_back(p2.polynomial_coefficients[j] *
                                             p2.polynomial_powers[j]);
...
T find_root(polynomial<T>& p2) {
  ...
  for (int64_t i = 1; i <= p2.degree; ++i) trials *= 10;
