//VAMOS CRIAR UM PROGRAMA QUE IMPLEMENTA UMA CLASSE DE FRAÇÕES (NÚMEROS RACIONAIS) PARA SE TRABALHAR COM FLOATING POINT NUMBERS USANDO APENAS INTEIROS
...
T rational_gcd(T a, T b){
  ...
  static_assert(std::is_integral<T>::value, "Parcelas devem ser números inteiros.\n");
...
rational_number<T>::rational_number(){
  //Restrição a tipos inteiros
  static_assert(std::is_integral<T>::value, "Numerador e denominador devem ser inteiros.\n");
                                     };
...
rational_number<T> rational_number<T>::operator+(rational_number<T> r1){
  ...
  //Procedimentos
    ...
    result.numerator=((result.denominator/denominator)*numerator)+((result.denominator/r1.denominator)*r1.numerator);
...
template<typename T>
T rational_number<T>::get_numerator(){ return numerator; };

template<typename T>
T rational_number<T>::get_denominator(){ return denominator; };
...
//********************************************************************************************************************************************************
//FUNÇÕES DE CONVERSÃO DA CLASSE DE RACIONAIS PARA INTEIROS E FLOATING POINT NUMBERS
