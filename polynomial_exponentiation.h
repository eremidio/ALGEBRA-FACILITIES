...
polynomial<T> polynomial_bin_pow(polynomial<T>& p1, U exponent){
 ...
 static_assert(std::is_integral<U>::value, "Coeficientes de polinômios devem ser inteiros");
...
template<typename T, typename U>
polynomial<T> polynomial_powmod(polynomial<T>& p1, polynomial<T>& p2, U exponent){
