...
polynomial<T> polynomial_modulus_reduction(polynomial<T>& p1, U modulus){
 ...
 static_assert(std::is_integral<U>::value, "Fatores de redução devem ser inteiros");
 static_assert(std::is_integral<T>::value, "Coeficiente dos polinômios de redução devem ser inteiros");
 ...
 //Procedimentos
  ...
  for(auto& x:p2.polynomial_coefficients)
   x%=modulus;
