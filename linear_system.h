// VAMOS CRIAR UM PROGRAMA QUE IMPLMENTA A CLASSE DE SOLUCIONADORES DE SISTEMAS
// LINEARES

/*

VAMOS IMPLMENTAR O ALGORITMO DE CRAMER PARA RESOLVER SISTEMAS DE EQUAÇÕES
LINEARES USANDO DETERMINANTES PARA MAIORES INFORMAÇÕES:
...
class cramer_linear_system_solver {
 public:
  ...
  const char symbols[27] = {'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i',
                            ...
                            's', 't', 'u', 'v', 'w', 'x', 'y', 'z'};
  Matrix<T>
      left_coefficients_matrix;  // Matriz de coeficientes das equações lineares
  ...
  T main_determinant;  // Variável que determina se um sistema é resolúvel por
                       // meio do algoritmo de Cramer
  ...
  // Métodos da classe
  void setup();  // Função que recebe input do usuário e ajusta as matrizes
                 // usadas no cálculo
  void set_status();  // Função que determina se um sistema é resolúvel ou não
  void print_linear_system();  // Função printa o sistema de equações lineares
                               // na tela
  ...
  void print_solution();  // Função printa a solução do sistema de equações
                          // lineares na tela
};
...
void cramer_linear_system_solver<T>::setup() {
  ...
  std::cout << "Usuário digite quantas equações e variáveis desconhecidas há "
               "no seu sistema linear: ";
  std::cin >> dimension;
...
// Função que determina se um sistema é resolúvel ou não por meio do algoritmo
// de Cramer
template <typename T>
void cramer_linear_system_solver<T>::set_status() {
  ...
  if (main_determinant == 0)
    std::cout << "O sistema de equações não admite soluções ou possui "
                 "infinitas soluções!\n";
};
    ...
    for (j = 0; j < dimension; ++j) {
      if (j == 0)
        ...
        if (left_coefficients_matrix.matrix[i][j] >= 0)
          std::cout << "+" << left_coefficients_matrix.matrix[i][j]
                    << symbols[j];
        else
...
void cramer_linear_system_solver<T>::solve() {
  ...
  // Procedimento
  for (i = 0; i < dimension; ++i) step_solution(i);
};
