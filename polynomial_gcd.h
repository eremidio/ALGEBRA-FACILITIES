...
bool is_equal_polynomial(polynomial<T>& p1, polynomial<T>& p2){
 ...
 //Resultado
 if(p2.degree==p1.degree && p1.polynomial_coefficients==p2.polynomial_coefficients)
  return true;
 else
  return false;
...
polynomial<T> polynomial_gcd(polynomial<T>& p1, polynomial<T>& p2){
 ...
 //Procedimentos 
  ...
  //Caso geral: p2(x) não divide p1(x)
    //Variáveis locais
    polynomial<double> u=polynomial_cast<T, double>(p2); //Aritmética será feita no domínio dos reais para evitar erros devido a float point exceptions
  ...
  while(1){
    ...
    polynomial<double> temp=remainder<double>(u, v);
        ...
        /*std::cout<<"u(x): "<<u.algebraic()<<'\n';
        std::cout<<"v(x): "<<v.algebraic()<<'\n';
        std::cout<<"temp(x): "<<temp.algebraic()<<'\n';*/

    if(is_zero_polynomial<double>(temp) || is_equal_polynomial<double>(u, v)==true)
     break;
    else{
     u=v;  
